...
import sys
sys.path.append(r"E:\mystery_ai\bronya_project\bronya_appearance")
...
class TransparentWindow(QLabel):
    def __init__(self, width, height):
        super().__init__()
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.SubWindow | Qt.WindowSystemMenuHint | Qt.WindowMinMaxButtonsHint | Qt.Window)
    ...
    def mouseMoveEvent(self, event):
        if self.drag_position and not self.resizing:
            self.move(event.globalPos() - self.drag_position)
            event.accept()
        elif self.resizing:
            #self.setFixedSize(event.pos().x(), event.pos().y())
...
class EasyAIV(Process):  #
    def __init__(self, model_process_args, alive_args):
        ...
        self.app = None  # 初始化 app 属性为 None


    @torch.no_grad()
    def run(self):
        IMG_WIDTH = 512
        self.app = QApplication(sys.argv)  # 在run中初始化 QApplication
        self.window = TransparentWindow(width=args.output_w, height=args.output_h)  # 在run中初始化 window
        ...
        while True:
            ...
            self.window.update_image(postprocessed_image)
